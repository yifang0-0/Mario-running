...
from .. components import info, inputbox
from source.components import database as db
...
class Menu(tools.State):
    ...
    def setup_login_in(self):
        self.userinfo = inputbox.ask(setup.SCREEN, "UserName", "PassWord")
        print("".join(self.userinfo[0]))
        print("".join(self.userinfo[1]))
        db.login("".join(self.userinfo[0]), "".join(self.userinfo[1]))
