...
class Player(pg.sprite.Sprite):
    ...
    def load_images(self):
        ...
        for name, frames in frames_list.items():
            for frame in frames:
                image = tool.get_image(sheet, frame['x'], frame['y'], 
    ...
    def check_to_allow_jump(self, keys):
        if not keys[tool.keybinding['jump']]:
            self.allow_jump = True
    
    def check_to_allow_fireball(self, keys):
        if not keys[tool.keybinding['action']]:
            self.allow_fireball = True
    ...
    def standing(self, keys, fire_group):
        ...
        if keys[tool.keybinding['action']]:
            if self.fire and self.allow_fireball:
                self.shoot_fireball(fire_group)

        if keys[tool.keybinding['down']]:
            self.update_crouch_or_not(True)

        if keys[tool.keybinding['left']]:
            self.facing_right = False
            self.update_crouch_or_not()
            self.state = c.WALK
        elif keys[tool.keybinding['right']]:
            self.facing_right = True
            self.update_crouch_or_not()
            self.state = c.WALK
        elif keys[tool.keybinding['jump']]:
            if self.allow_jump:
                self.state = c.JUMP
                self.y_vel = self.jump_vel
        
        if not keys[tool.keybinding['down']]:
            self.update_crouch_or_not()
    ...
    def jumping(self, keys, fire_group):
        ...
        if keys[tool.keybinding['right']]:
            self.x_vel = self.cal_vel(self.x_vel, self.max_x_vel, self.x_accel)
        elif keys[tool.keybinding['left']]:
            self.x_vel = self.cal_vel(self.x_vel, self.max_x_vel, self.x_accel, True)
